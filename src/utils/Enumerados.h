...
enum TipoEntidad  { OTROS, PASTO, TIERRA, AGUA, ARBOL, CASTILLO, SOLDADO, ARQUERO, ALDEANO, ANIMAL, MADERA, COMIDA, PIEDRA, ORO, DEFAULT, EDIFICIO, CONSTRUCCION, CENTRO_CIVICO, BARRACK_1, BARRACK_2, BARRACK_3, CUARTEL, MINA_ORO, MINA_PIEDRA };
...
inline const bool EsEdificio(TipoEntidad tipo) {
	if (tipo == EDIFICIO || tipo == CONSTRUCCION || tipo == CASTILLO || tipo == CUARTEL || tipo == CENTRO_CIVICO
		|| tipo == BARRACK_1 || tipo == BARRACK_2 || tipo == BARRACK_3)
		return true;
	return false;
}
