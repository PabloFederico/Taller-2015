...
enum TipoEntidad  { OTROS , PASTO, TIERRA , AGUA , ARBOL , CASTILLO, SOLDADO, ARQUERO, ALDEANO, ANIMAL, MADERA, COMIDA, PIEDRA, ORO, DEFAULT, EDIFICIO, CONSTRUCCION, CENTRO_CIVICO, BARRACK_1, BARRACK_2, BARRACK_3, CUARTEL };
...
enum TipoImagenUtil { CUADRADO_MAGENTA, CUADRADO_ROJO, CUADRADO_AZUL, CUADRADO_VERDE,
					...
					BARRACK_1_TRANS, BARRACK_1_ROJIZO, BARRACK_2_TRANS, BARRACK_2_ROJIZO,
					BARRACK_3_TRANS, BARRACK_3_ROJIZO, CUARTEL_TRANS, CUARTEL_ROJIZO};
...
enum TipoSonido { CAMINAR, MORIR_HUMANO, MORIR_ANIMAL, DESTRUIR };
...
inline const bool EsEdificio(TipoEntidad tipo) {
	if (tipo == EDIFICIO || tipo == CASTILLO || tipo == CUARTEL ||
		tipo == CENTRO_CIVICO || tipo == BARRACK_1 || tipo == BARRACK_2 || tipo == BARRACK_3)
		return true;
	return false;
}
